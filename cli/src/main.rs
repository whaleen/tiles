    ...
    for (index, (file, base_dir)) in targets.into_iter().enumerate() {
        ...
        let target = if overwrite {
            ...
            let ext = file
                ...
                .unwrap_or_else(|| ".mp4".to_string());
            out_dir.join(format!("{stem}_cfr{ext}"))
        } else {
    ...
    for (index, (file, base_dir)) in targets.into_iter().enumerate() {
        ...
        let target = if overwrite {
            ...
            let ext = file
                ...
                .map(|s| format!(".{}", s.to_string_lossy()))
                .unwrap_or_else(|| ".mp4".to_string());
            out_dir.join(format!("{stem}_trimmed{ext}"))
        } else {
            out_dir.join(file.file_name().unwrap_or_default())
        };
        let mut pipeline = FFmpegPipeline::new(base);
    ...
    for (index, (file, base_dir)) in targets.into_iter().enumerate() {
        ...
        let target = if overwrite {
            ...
            let ext = file
                ...
                .map(|s| format!(".{}", s.to_string_lossy()))
                .unwrap_or_else(|| ".mp4".to_string());
            out_dir.join(format!("{stem}_slowmo{ext}"))
        } else {
            out_dir.join(file.file_name().unwrap_or_default())
        };
    ...
    for (index, (file, base_dir)) in targets.into_iter().enumerate() {
        ...
        let target = if overwrite {
            ...
            let ext = file
                ...
                .map(|s| format!(".{}", s.to_string_lossy()))
                .unwrap_or_else(|| ".mp4".to_string());
            out_dir.join(format!("{stem}_cropped{ext}"))
        } else {
            out_dir.join(file.file_name().unwrap_or_default())
        };
...
fn find_root_from(start: &Path) -> Option<PathBuf> {
    ...
    loop {
        ...
        let is_workspace = cur.join("src").is_dir()
            && cur.join("outputs").is_dir()
